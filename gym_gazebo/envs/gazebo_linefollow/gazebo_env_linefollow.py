...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def line_detected_in_bin(self, bin_image):
        ...
        bin_image_hsv = cv2.cvtColor(bin_image, cv2.COLOR_BGR2HSV)

        binary_mask = cv2.inRange(bin_image_hsv, lower_blue, upper_blue)

        contours, hierarchy = cv2.findContours(binary_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        filtered_contours = [contour for contour in contours if cv2.contourArea(contour)>1]

        line_detected = bool(filtered_contours)
    ...
    def process_image(self, data):
        ...
        for i in range(10):
            ...
            bin_image = cv_image[:,start_col:end_col]

            if self.line_detected_in_bin(bin_image):
                state[i] = 1
